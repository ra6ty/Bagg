...
Изначально есть пара полезных вещей:
 ...
 ## Верстка
Команды для сборки:
 - `npm run dev` запускает сборку и локальный сервер с Hot Reloading
 - `npm run prod` запускает сборку и на выходе получаем собранные, минифицированные файлы стилей и скриптов
